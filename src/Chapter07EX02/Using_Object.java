...
class A {   //클래스 : 객체를 생성하기 위한 설계도 
	// 클래스 내부에 올 수 있는것(4가지) : 1. 필드 (클래스에서 선언된 변수), 2. 생성자 , Method, 4. 인너클래스
	// 1. 필드 
	...
	// 생성자 : 클래스는 반드시 생성자가 존재해야 한다. 특징 : 1.클래스 이름과 동일, 2. 리턴이 존재하지 않는다.  생성자는 무조건 클래스내에 있어야한다.
	//       기본 생성자(입력 매개변수 가 없는 생성자)는 생략 될 수 있다. A () {} 
    //       객체 생성시 생성자 호출
	//       객체의 초기값을 셋팅
	//       기본 생성자는 생략되어 있기 경우 컴파일러가 자동으로 등록해 준다.
	//       클래스 내부의 다른 생성자가 존재할 시 컴파일러가 기본 생성자를 자동으로 만들어 주지 않는다.
	A () {
		 System.out.println("기본 생성자 호출");
	}
	
	//메소드 :  리턴타입 메소드명 (입력 매개 변수) {실행코드; } 
	//    프로그램의 기능을 코드로 작성
	//    호출해서 작동됨, work1()
...
public class Using_Object {
	...
	public static void main(String[] args) {
		...
		//A() : 클래스 내부의 생성자 호출, 기본 생성자 : 입력 매개 변수가 없는 생성자 
		...
		double d = a1.work3(10, 20.0, "오늘");  //리턴받은 값으 변수에 할당 후 출력
